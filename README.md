...
| 模块     | 已实现                        | 规划中                       |
| -------- | ----------------------------- | ---------------------------- |
| 核心引擎 | ✅ LLM自动回复<br>✅ 上下文管理<br>✅ 推理模型支持（自动过滤`<think>`标签） | 🔄 情感分析增强               |
| 议价系统 | ✅ 阶梯降价策略                | 🔄 市场比价功能               |
| 技术支持 | ✅ 网络搜索整合                | 🔄 RAG知识库增强              |
| 订单管理 | ✅ 交易成功自动发货回复         | 🔄 物流状态推送               |
...
| 变量 | 必填 | 默认值 | 说明 |
|---|---|---|---|
| `API_KEY` | 是 | — | LLM API Key（默认使用阿里云百炼） |
| `COOKIES_STR` | 是 | — | 闲鱼网页端 Cookie 字符串 |
| `MODEL_BASE_URL` | 否 | `https://dashscope.aliyuncs.com/compatible-mode/v1` | OpenAI 兼容接口地址 |
| `MODEL_NAME` | 否 | `qwen-max` | 模型名称 |
| `TOGGLE_KEYWORDS` | 否 | `。` | 人工/AI 模式切换关键词 |
| `SIMULATE_HUMAN_TYPING` | 否 | `False` | 模拟人工输入延迟 |
| `LOG_LEVEL` | 否 | `DEBUG` | 日志级别 |
| `HEARTBEAT_INTERVAL` | 否 | `15` | WebSocket 心跳间隔（秒） |
| `HEARTBEAT_TIMEOUT` | 否 | `5` | 心跳响应超时时间（秒） |
| `TOKEN_REFRESH_INTERVAL` | 否 | `3600` | Token 刷新间隔（秒） |
| `TOKEN_RETRY_INTERVAL` | 否 | `300` | Token 刷新失败重试间隔（秒） |
| `MANUAL_MODE_TIMEOUT` | 否 | `3600` | 人工接管自动退出超时（秒） |
| `MESSAGE_EXPIRE_TIME` | 否 | `300000` | 忽略超过此时长的历史消息（毫秒） |

### 订单自动回复配置

交易成功（买家付款后）可根据商品关键词自动发送发货通知，编辑 `order_reply_config.json`：
...
```json
{
  "items": [
    ...
    {
      "keywords": ["耳机", "AirPods"],
      "reply": "感谢您的购买！耳机已安排发货，有问题欢迎随时联系。"
    }
  ],
  "default_reply": "感谢您的购买！祝您使用愉快，有问题欢迎随时联系。"
}
```

- `keywords`：商品标题匹配关键词，命中任意一个即触发对应回复
- `default_reply`：无关键词匹配时的兜底回复，留空则不发送
...
<div align="center">
  <table>
    <tr>
      <td align="center"><strong>交流群21（已满200）</strong></td>
      <td align="center"><strong>交流群22（推荐加入）</strong></td>
    </tr>
    <tr>
      <td><img src="./images/wx_group21.png" width="300px" alt="交流群21"></td>
