...
COOKIES_STR可通过扫码登录自动获取，也可以自行在闲鱼网页端获取cookies(网页端F12打开控制台，选择Network，点击Fetch/XHR,点击一个请求，查看cookies)
...
首次登录或Cookie失效时，可以先运行扫码登录，成功后会自动写入 `.env`：
```bash
python main.py --qr-login
```

也可以直接运行主程序；如果 `COOKIES_STR` 为空或仍是占位符，按提示直接回车即可进入扫码登录流程：
